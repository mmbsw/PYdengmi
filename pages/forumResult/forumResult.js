...
Page({
  ...
  //删除事件
  delete: function (e) {
    var _forumvalue = this.data.forumvalue;
      ...
      success: function (res) {
        wx.showToast({
          ...
          success:function(){
            setTimeout(function(){
              wx.redirectTo({
                url: '../forumResult/forumResult?forumvalue='+_forumvalue,
              })
            },500);
          }
        })
  ...
  onLoad: function (options) {
    console.log(options)
    var searchValue = options.forumvalue ? options.forumvalue : options;
    var that = this;
    var mihao = wx.getStorageSync('mihao');
      that.setData({
        mihao: mihao,
        forumvalue: searchValue,
      })
   //设置标题栏
    wx.setNavigationBarTitle({
      title: '“帖子标题中”包含“' + searchValue + '”',
    })
  },
  ...
  onShow: function () {
    var that = this;
    var tips =  [{publisher: "提示", speech: "没有搜索到相关内容"}];
    wx.request({
      url: config.mobileHost+'/getForumTitle',
      data:{
        'searchValue': that.data.forumvalue, 
      },
      method:'get',
      header:{
        'content-type': 'application/json'
